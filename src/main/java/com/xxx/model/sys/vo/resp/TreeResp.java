...
public class TreeResp {
    ...
    public static TreeResp getTreeDepartmentResp(List<SysDepartmentResp> sysDepartmentResps){
        TreeResp tree = new TreeResp();
        tree.setId(0L);
        tree.setSpread(true);
        tree.setAlias("0");
        tree.setName("主菜单");
        tree.setChildren(getTreeDepartmentResps(sysDepartmentResps));
        return tree;
    }

    public static List<TreeResp> getTreeDepartmentResps(List<SysDepartmentResp> sysDepartmentResps){
        List<TreeResp> list = sysDepartmentResps.stream().map(department -> {
            TreeResp tree = new TreeResp();
            tree.setId(department.getId());
            tree.setName(department.getName());
            tree.setSpread(true);
            tree.setAlias(department.getLevel().toString());
            if (department.getSysDepartmentResps() != null && department.getSysDepartmentResps().size() > 0) {
                tree.setChildren(getTreeDepartmentResps(department.getSysDepartmentResps()));
            }
            return tree;
        }).collect(Collectors.toList());
        return list;
