                ...
                <table class="layui-table">
                    ...
                    <tr>
                        <td>${(entity_index+1)!}</td>
                                <td><a href="javascript:layui.showInfo('${(entity.id)!}')">${(entity.managerTelephone)!}</a></td>
                                <td>${(entity.managerPassword)!}</td>
                                <td>${(entity.managerLimit)!}</td>
                                <td>${(entity.managerCondition)!}</td>
                        <@common.permission per='base:ddManager:edit'>
                        <td>
        ...
        <script>
            layui.use(['form','laydate','laypage','layer','upload'],function(){
                ...
                layui.showInfo = function(id){
                    layui.save("查看模块管理",id);
                }

                //编辑
                layui.edit = function(id){
                    layui.save("编辑模块管理",id)
                }
                ...
                layui.save = function(title,id){
                    var url ='${springMacroRequestContext.contextPath}/ddManager/saveFrom';
                ...
                layui.accredit = function(id,status){
                    ...
                    layer.msg(title, {
                        btn: b,
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/ddManager/accredit?id="+id+"&status="+status;
                        }
                    });
                }
                ...
                layui.del = function(id){
                    layer.msg("您是否确定要删除该模块管理！", {
                        btn: ['确定', '取消'],
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/ddManager/delById?id="+id;
                        }
                    });
                }
            });
