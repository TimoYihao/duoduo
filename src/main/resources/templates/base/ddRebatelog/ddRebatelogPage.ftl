                ...
                <table class="layui-table">
                    ...
                    <tr>
                        <td>${(entity_index+1)!}</td>
                                <td><a href="javascript:layui.showInfo('${(entity.id)!}')">${(entity.userTelephoneF)!}</a></td>
                                <td>${(entity.userTelephoneS)!}</td>
        ...
        <script>
            layui.use(['form','laydate','laypage','layer','upload'],function(){
                ...
                layui.showInfo = function(id){
                    layui.save("查看积分日志",id);
                }

                //编辑
                layui.edit = function(id){
                    layui.save("编辑积分日志",id)
                }
                ...
                layui.save = function(title,id){
                    var url ='${springMacroRequestContext.contextPath}/ddRebatelog/saveFrom';
                ...
                layui.accredit = function(id,status){
                    ...
                    layer.msg(title, {
                        btn: b,
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/ddRebatelog/accredit?id="+id+"&status="+status;
                        }
                    });
                }
                ...
                layui.del = function(id){
                    layer.msg("您是否确定要删除该积分日志！", {
                        btn: ['确定', '取消'],
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/ddRebatelog/delById?id="+id;
                        }
                    });
                }
            });
