                ...
                <table class="layui-table">
                    ...
                    <tr>
                        <td>${(entity_index+1)!}</td>
                                <td><a href="javascript:layui.showInfo('${(entity.id)!}')">${(entity.user)!}</a></td>
                                <td>${(entity.name)!}</td>
                                <td>${(entity.phone)!}</td>
                                <td>${(entity.imgA)!}</td>
                                <td>${(entity.imgB)!}</td>
        ...
        <script>
            layui.use(['form','laydate','laypage','layer','upload'],function(){
                ...
                layui.showInfo = function(id){
                    layui.save("查看身份信息",id);
                }

                //编辑
                layui.edit = function(id){
                    layui.save("编辑身份信息",id)
                }
                ...
                layui.save = function(title,id){
                    var url ='${springMacroRequestContext.contextPath}/identity/saveFrom';
                ...
                layui.accredit = function(id,status){
                    ...
                    layer.msg(title, {
                        btn: b,
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/identity/accredit?id="+id+"&status="+status;
                        }
                    });
                }
                ...
                layui.del = function(id){
                    layer.msg("您是否确定要删除该身份信息！", {
                        btn: ['确定', '取消'],
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/identity/delById?id="+id;
                        }
                    });
                }
            });
