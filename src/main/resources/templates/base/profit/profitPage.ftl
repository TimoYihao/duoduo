                ...
                <table class="layui-table">
                    ...
                    <tr>
                        <td>${(entity_index+1)!}</td>
                                <td><a href="javascript:layui.showInfo('${(entity.id)!}')">${(entity.sum)!}</a></td>
                                <td><#if entity.time?? >${entity.time?string("yyyy年MM月dd日 HH:mm:ss")}</#if></td>
                                <td>${(entity.type)!}</td>
                                <td>${(entity.signing)!}</td>
                        <@common.permission per='base:profit:edit'>
                        <td>
        ...
        <script>
            layui.use(['form','laydate','laypage','layer','upload'],function(){
                ...
                layui.showInfo = function(id){
                    layui.save("查看签约收益",id);
                }

                //编辑
                layui.edit = function(id){
                    layui.save("编辑签约收益",id)
                }
                ...
                layui.save = function(title,id){
                    var url ='${springMacroRequestContext.contextPath}/profit/saveFrom';
                ...
                layui.accredit = function(id,status){
                    ...
                    layer.msg(title, {
                        btn: b,
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/profit/accredit?id="+id+"&status="+status;
                        }
                    });
                }
                ...
                layui.del = function(id){
                    layer.msg("您是否确定要删除该签约收益！", {
                        btn: ['确定', '取消'],
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/profit/delById?id="+id;
                        }
                    });
                }
            });
