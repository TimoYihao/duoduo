            ...
            <form id="searchForm" class="layui-form" method="post" style="margin-top: 20px;">
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline">
                        <input  type = "text" name="department" value="${(salestarget.department)!}" placeholder="所属部门" class="layui-input" />
                    </div>
                    <div class="layui-inline">
                ...
                <table class="layui-table">
                    ...
                    <tr>
                        <td>${(entity_index+1)!}</td>
                                <td><a href="javascript:layui.showInfo('${(entity.id)!}')">${(entity.department)!}</a></td>
                                <td>${(entity.sum)!}</td>
                                <td><#if entity.start?? >${entity.start?string("yyyy年MM月dd日 HH:mm:ss")}</#if></td>
                                <td><#if entity.end?? >${entity.end?string("yyyy年MM月dd日 HH:mm:ss")}</#if></td>
                                <td>${(entity.rate)!}</td>
        ...
        <script>
            layui.use(['form','laydate','laypage','layer','upload'],function(){
                ...
                layui.showInfo = function(id){
                    layui.save("查看销售目标",id);
                }

                //编辑
                layui.edit = function(id){
                    layui.save("编辑销售目标",id)
                }
                ...
                layui.save = function(title,id){
                    var url ='${springMacroRequestContext.contextPath}/salestarget/saveFrom';
                ...
                layui.accredit = function(id,status){
                    ...
                    layer.msg(title, {
                        btn: b,
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/salestarget/accredit?id="+id+"&status="+status;
                        }
                    });
                }
                ...
                layui.del = function(id){
                    layer.msg("您是否确定要删除该销售目标！", {
                        btn: ['确定', '取消'],
                        yes:function(){
                            window.location.href="${springMacroRequestContext.contextPath}/salestarget/delById?id="+id;
                        }
                    });
                }
            });
