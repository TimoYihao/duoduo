    ...
    $.get('/newsList?id='+id+'&table=mt_qyrz&limit=0,6&type=0&posids=5', function(res){
        res.list.forEach(function (item,index) {
            if(index<2){
                ...
                $(".hyzx").append('<li class="bg_new_sm"><a href="/details?id='+item.id+'&type=4">'+item.fbt+'</a></li>');
            }
        });
    });
}

function getJdalList() {
    var id = $('input[name="jdal"]').val();
    $.get('/newsList?id='+id+'&table=mt_qyrz&limit=0,6&type=0&posids=5', function(res){
        res.list.forEach(function (item,index) {
            if(index<2){
                $(".jdal").append('<li class="pull-left">' +
                    '<div class="pull-left content_mid_img tv_hot f1">' +
                ...
                $(".jdal").append('<li class="bg_new_sm"><a href="/details?id='+item.id+'&type=4">'+item.fbt+'</a></li>');
            }
        });
    });
}